...
Now, you can run the CLI with the comment `section` and `message` which will be posted.

```bash
npx @omni-comment/cli \
  --token "$GITHUB_TOKEN" \
  --repo "owner/repo" \
  --issue-number 123 \
...
To comment from stdin, you can use the `--stdin` argument instead of `message`.
...
| `--issue-number` | The pull request number where to create the comment              | ✅       |                    |
| `--section`      | The section ID that matches with the value in `omni-comment.yml` | ✅       |                    |
| `--message`      | Comment body                                                     |          |                    |
| `--collapsed`    | Make comment collapsed                                           |          | `false`            |
